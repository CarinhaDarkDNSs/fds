	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'audio':
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return client.sendMessage(from, '✖ (Ex: ${prefix}audio pt-br Seu texto)', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '❌ Cade texto mula?❌', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('❌ Cade texto mula?❌')
				...
				case 'info':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgbb.com/"><img src="https://i.ibb.co/wgQ0fyH/Clientes-Blaze-Hosting-20210110-123829.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*💙 BlazeHosting 💙*\n\n*🌐 Site:* www.blazehosting.com.br\n*💻 Twitter:* twitter.com/blazehostingbr\n*📲 Discord:* www.discord.gg/ATkB2ws\n\n*⌨ Painel Servidores:* www.central.blazebr.xyz/painel.apk'})
					break
				case 'belgle2':
					memein = await kagApi.memeindo()
				...
				case 'hilih':
					...
					anu = await fetchJson(`https://oc-129-150-70-222.compute.oraclecloud.com/rest/venda-online/v2/propostas/ConsultaBaseCpf?cpf='.$cpf.'`, {method: 'get'})
					reply(anu.result)
					break
				case 'yt2mp3':
